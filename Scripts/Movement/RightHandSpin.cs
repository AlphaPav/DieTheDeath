...
public class RightHandSpin : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Global.state == Global.READY_SPIN_R)
        {
            // 固定右手
            ...
            else
            {
                ...
                // 开始旋转
                Global.state = Global.SPIN_R;
            }
        }
        else if (Global.state == Global.SPIN_R)
        {
            Debug.Log("正在旋转");
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag.Equals("Touchable"))
        {
            ...
            {
                // Update state
                Global.state = Global.READY_SPIN_R;
                Global.hand_state_right = Global.NORMAL_R;   // 静止
                Global.hand_state_left = Global.SHORTEN_L;   // 左手恢复
